...
public class BubblesortPara {
  ...
  int[] arr;
  int segmentLength;
  BubblesortPara(int max) {} // end constructor BubblesortPara
  ...
  void doit(BubblesortPara b, int num, int threadCount) throws InterruptedException {
    ...
    //System.out.println("Segment length: "+segmentLength+" segment count: "+segmentCount);
    ...
    System.out.println("Bubblesort Parallel time " + (paraTime / 1000000.0) + " ms\nSpeedup " + seqTime * 1.0 / paraTime);
  }
    ...
    public void run() { // Her er det som kjores i parallell:
      ...
      while (passId.get() > 1) {
        ...
        for (int s = 0; s <= myLastSeg; s++) {
          ...
          //System.out.println("T"+ind+" starting seg "+s+" segEnd: "+segEnd+" myLastSeg "+myLastSeg);
